...
test('WebKit iPhone landscape keeps editor interactive without runtime errors',async({page,browserName})=>{
  ...
  await page.locator('button[data-motion-open]').click();await expect(page.locator('.motion-studio')).toBeVisible();await page.locator('button[data-motion-close]').click();
  await page.locator('button[data-cutscene-open]').click();await expect(page.locator('.cutscene-studio')).toBeVisible();await page.locator('button[data-cutscene-close]').click();
