...
test('canvas2d fallback visually audits all 92 generated source-sheet editor parts',async({page})=>{
  ...
  const allParts={
    outfit:['hooded','high-collar','zip-collar','drawstring','short-sleeve','vest'],
    ...
    hair:['ponytail','bob','side-tail','twin-tail','braid','long','wavy','short-spike','bun','half-up'],
    face:['soft','oval','angular','round','square','pointed','long-oval','hex','diamond','tapered'],
    eye:['bright','determined','sharp','round','soft','sleepy','sparkle','closed','narrow','side-glance'],
    brow:['soft','straight','angled','thin','bold','arched','calm','raised','flat','worried'],
    nose:['diamond','small','line','soft','tall','tiny','faceted','profile','wide','button'],
    mouth:['smile-open','smile','neutral','soft-smile','o','surprised','smirk','frown','wide-open','curve'],
  } as const;
  let audited=0;
  for(const[kind,ids]of Object.entries(allParts))for(const id of ids){
    const option=page.locator(`[data-kind="${kind}"][data-id="${id}"]`);await expect(option).toHaveCount(1);await option.click();await expect(option).toHaveClass(/selected/);
    await canvas.screenshot({path:`visual-audit/output/full-part-${kind}-${id}.png`});audited++;
  }
