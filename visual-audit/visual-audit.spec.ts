...
test('Canvas2D full-editor audit covers and auto-fits all 92 generated parts',async({page})=>{
  ...
  for(let i=0;i<20;i++){
    ...
    for(const entry of report.entries){expect(Number.isFinite(entry.score)).toBe(true);seen.add(`${entry.family}:${entry.id.replace(/:(left|right)$/,'')}`);if(entry.family==='hair')expect(entry.score).toBeLessThan(4);else expect(entry.score).toBeLessThan(.8);}reports.push({combination:i+1,metrics,report});const n=String(i+1).padStart(2,'0');await canvas.screenshot({path:`visual-audit/output/autofit-preview-${n}.png`});await page.screenshot({path:`visual-audit/output/autofit-editor-${n}.png`,fullPage:true});
  }
