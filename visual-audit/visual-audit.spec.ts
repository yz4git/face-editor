...
type AuditReport={version:number;entries:{id:string;family:string;score:number}[]};
...
test('Canvas2D full-editor audit covers and auto-fits all 92 generated parts',async({page})=>{
  mkdirSync('visual-audit/output',{recursive:true});
  ...
  for(let i=0;i<10;i++){
    ...
    const report=await page.evaluate(()=>((window as Window&{__FACE_EDITOR_AUTOFIT_REPORT__?:AuditReport}).__FACE_EDITOR_AUTOFIT_REPORT__));
    expect(report?.version).toBe(2);expect(report?.entries).toHaveLength(13);if(!report)throw new Error('Missing auto-fit report');
    for(const entry of report.entries){expect(Number.isFinite(entry.score)).toBe(true);seen.add(`${entry.family}:${entry.id.replace(/:(left|right)$/,'')}`);if(entry.family==='hair')expect(entry.score).toBeLessThan(4);else expect(entry.score).toBeLessThan(.8);}
    reports.push({combination:i+1,metrics,report});const n=String(i+1).padStart(2,'0');await canvas.screenshot({path:`visual-audit/output/autofit-preview-${n}.png`});await page.screenshot({path:`visual-audit/output/autofit-editor-${n}.png`,fullPage:true});
  }
  expect(seen.size).toBe(92);writeFileSync('visual-audit/output/autofit-report.json',JSON.stringify({version:2,seenParts:[...seen].sort(),seenCount:seen.size,combinations:reports},null,2));
