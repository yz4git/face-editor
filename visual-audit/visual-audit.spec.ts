...
function scoreCanvasAgainstReference(element:Element,key:string):VisualDifferenceMetrics{
  const canvas=element as HTMLCanvasElement,ctx=canvas.getContext('2d');if(!ctx)throw new Error('Canvas2D context unavailable');
  ...
  const current=ctx.getImageData(0,0,canvas.width,canvas.height).data,stride=2,w=Math.ceil(canvas.width/stride),h=Math.ceil(canvas.height/stride),diffMask=new Uint8Array(w*h),refMask=new Uint8Array(w*h),curMask=new Uint8Array(w*h);
  ...
  for(let by=0;by<h;by++)for(let bx=0;bx<w;bx++){
    ...
    const p=by*w+bx;if(refOn){refMask[p]=1;refX+=bx;refY+=by;refCount++;refMinX=Math.min(refMinX,bx);refMinY=Math.min(refMinY,by);refMaxX=Math.max(refMaxX,bx);refMaxY=Math.max(refMaxY,by);}if(curOn){curMask[p]=1;curX+=bx;curY+=by;curCount++;curMinX=Math.min(curMinX,bx);curMinY=Math.min(curMinY,by);curMaxX=Math.max(curMaxX,bx);curMaxY=Math.max(curMaxY,by);}if(refOn||curOn)unionCount++;
  ...
  return{differenceRatio:unionCount?diffCount/unionCount:0,meanColorDelta:colorSamples?totalColorDelta/colorSamples:0,centroidShift:Math.hypot(curCx-refCx,curCy-refCy)/charSpan,bboxScaleDeviation:Math.abs(Math.log(curWidth/refWidth))+Math.abs(Math.log(curHeight/refHeight)),longestDiffSpan,thinSpike,edgeTouchRatio:diffCount?edgeDiff/diffCount:0};
}

test('canvas2d fallback visually audits and scores all 92 generated source-sheet editor parts',async({page})=>{
  ...
  const report:{family:string;referenceId:string;results:ReturnType<typeof scoreVisualAnomalyFamily>}[]=[],critical:string[]=[];let audited=0;
  for(const[kind,ids]of Object.entries(allParts) as [keyof typeof allParts,readonly string[]][]){
    const referenceId=references[kind];await page.locator(`[data-kind="${kind}"][data-id="${referenceId}"]`).click();await canvas.evaluate(storeCanvasReference,kind);const samples=[];
    for(const id of ids){const option=page.locator(`[data-kind="${kind}"][data-id="${id}"]`);await expect(option).toHaveCount(1);await option.click();await expect(option).toHaveClass(/selected/);await canvas.screenshot({path:`visual-audit/output/full-part-${kind}-${id}.png`});const metrics=await canvas.evaluate(scoreCanvasAgainstReference,kind);if(id!==referenceId)samples.push({id,metrics});audited++;}
  ...
  expect(audited).toBe(92);await fs.writeFile('visual-audit/output/anomaly-report.json',JSON.stringify({generatedAt:new Date().toISOString(),families:report,critical},null,2));
