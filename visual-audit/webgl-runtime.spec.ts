...
test('default desktop runtime uses WebGL and survives editing workflows',async({page})=>{
  ...
  await page.locator('.top-actions button[data-motion-open]').click();await page.locator('button[data-motion-pose="fight"]').click();await page.locator('button[data-motion-action="wave"]').click();await page.waitForTimeout(180);
  await page.locator('.expression-bar [data-expression="happy"]').click();await expect(page.locator('.expression-bar [data-expression="happy"]')).toHaveClass(/selected/);
  await page.locator('button[data-motion-close]').click();await page.locator('.top-actions button[data-cutscene-open]').click();await page.locator('button[data-cutscene-template="battle"]').click();await expect(page.locator('.cutscene-studio')).toBeVisible();
