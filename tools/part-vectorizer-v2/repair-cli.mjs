...
import { compareRepairToBaseline, REPAIR_PROFILES, selectRepairTargets } from './lib/repair.mjs';
...
const ids=new Set(targets.map(target=>target.id)),items=(manifest.items??[]).filter(item=>ids.has(item.id));
if(!items.length)throw new Error(`Anomaly report selected ${targets.length} target(s), but none matched manifest item ids.`);
const missing=[...ids].filter(id=>!items.some(item=>item.id===id));
const profileIds=[...new Set(targets.flatMap(target=>target.profiles))],profiles=[...DEFAULT_PROFILES,...profileIds.map(id=>REPAIR_PROFILES[id]).filter(Boolean)];
...
const decisions=compareRepairToBaseline(result.summary,baselineSummary),accepted=decisions.filter(v=>v.accepted),rejected=decisions.filter(v=>!v.accepted),repairSummary={schemaVersion:1,generatedAt:new Date().toISOString(),triggerScore:options.trigger,targets,missingManifestIds:missing,profileIds,decisions,accepted:accepted.map(v=>v.id),rejected:rejected.map(v=>v.id),output:result.outputRoot};
await fs.writeFile(path.join(result.outputRoot,'repair-summary.json'),JSON.stringify(repairSummary,null,2));
console.log(JSON.stringify({ok:rejected.length===0,targets:targets.map(v=>v.id),profiles:profileIds,accepted:repairSummary.accepted,rejected:repairSummary.rejected,output:result.outputRoot},null,2));
