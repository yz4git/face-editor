...
export function validateManifest(manifest){
  ...
  if(manifest.grid){
    for(const key of ['columns','rows'])assert(Number.isInteger(Number(manifest.grid[key]))&&Number(manifest.grid[key])>0,`grid.${key} must be a positive integer`);
    for(const key of ['x','y','width','height','gapX','gapY'])if(manifest.grid[key]!==undefined)assert(finite(manifest.grid[key]),`grid.${key} must be numeric`);
    if(manifest.grid.width!==undefined)assert(Number(manifest.grid.width)>0,'grid.width must be > 0');
    if(manifest.grid.height!==undefined)assert(Number(manifest.grid.height)>0,'grid.height must be > 0');
  }
  ...
  for(const[index,item]of manifest.items.entries()){
    ...
    assert(item.rect||manifest.grid,`items[${index}] requires rect or a root grid`);
