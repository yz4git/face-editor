...
import { CLOTHING_PACK_V1_IDS,clothingPackV1Triangles } from '../src/data/clothingVariationPackV1Geometry';
...
describe('generated source-sheet polygon geometry',()=>{
  ...
  it('keeps the image-derived clothing pack complete and finite',()=>{
    const counts=Object.values(CLOTHING_PACK_V1_IDS).map(ids=>ids.length);expect(counts).toEqual([6,6,6,6,6]);
    for(const [kind,ids] of Object.entries(CLOTHING_PACK_V1_IDS) as [keyof typeof CLOTHING_PACK_V1_IDS,readonly string[]][]){for(const id of ids){const triangles=clothingPackV1Triangles(kind,id);expect(triangles.length,`${kind}:${id}`).toBeGreaterThan(0);for(const item of triangles){expect(item.points.flat().every(Number.isFinite),`${kind}:${id}`).toBe(true);expect(Number.isFinite(item.shade)).toBe(true);}}}
  });

  it('exposes every source and clothing-pack variant as a selectable editor part',()=>{
    expect(Object.keys(HAIR_PARTS)).toHaveLength(10);expect(Object.keys(EYE_PARTS)).toHaveLength(10);expect(Object.keys(FACE_PARTS)).toHaveLength(10);expect(Object.keys(BROW_PARTS)).toHaveLength(10);expect(Object.keys(NOSE_PARTS)).toHaveLength(10);expect(Object.keys(MOUTH_PARTS)).toHaveLength(10);expect(Object.keys(OUTFIT_PARTS)).toHaveLength(12);expect(Object.keys(HOOD_PARTS)).toHaveLength(12);expect(Object.keys(SHIRT_PARTS)).toHaveLength(12);expect(Object.keys(STRAP_PARTS)).toHaveLength(12);expect(Object.keys(ACCENT_PARTS)).toHaveLength(14);
  ...
  it('preserves semantic layers in generated artwork and modular outfit rows',()=>{
    ...
    expect(Object.values(EYE_PARTS).filter(eye=>eye.triangles.some(t=>t.layer==='eye-white')).length).toBeGreaterThanOrEqual(8);expect(Object.values(EYE_PARTS).filter(eye=>eye.triangles.some(t=>t.layer==='eye-glint')).length).toBe(10);
    ...
    for(const hood of Object.values(HOOD_PARTS))expect(hood.triangles.every(t=>t.layer==='hood')).toBe(true);
    for(const shirt of Object.values(SHIRT_PARTS))expect(shirt.triangles.every(t=>t.layer==='shirt')).toBe(true);
    for(const accent of Object.values(ACCENT_PARTS)){expect(accent.triangles.some(t=>t.layer==='accent')).toBe(true);expect(accent.triangles.every(t=>t.layer==='accent'||t.layer==='strap-metal')).toBe(true);}
    expect(Object.values(STRAP_PARTS).some(strap=>strap.triangles.some(t=>t.layer==='strap-metal'))).toBe(true);expect(MOUTH_PARTS['smile-open'].triangles.some(t=>t.layer==='mouth-detail')).toBe(true);
  });
