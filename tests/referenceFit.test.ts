...
import { BROW_PARTS,EYE_PARTS,FACE_PARTS,HAIR_PARTS,MOUTH_PARTS,NOSE_PARTS,OUTFIT_PARTS } from '../src/data/partLibrary';
...
describe('generated source-sheet polygon geometry',()=>{
  ...
  it('exposes every generated source variant as a selectable editor part',()=>{
    expect(Object.keys(HAIR_PARTS)).toHaveLength(10);expect(Object.keys(EYE_PARTS)).toHaveLength(10);expect(Object.keys(FACE_PARTS)).toHaveLength(10);expect(Object.keys(BROW_PARTS)).toHaveLength(10);expect(Object.keys(NOSE_PARTS)).toHaveLength(10);expect(Object.keys(MOUTH_PARTS)).toHaveLength(10);expect(Object.keys(OUTFIT_PARTS)).toHaveLength(6);
    for(const family of generatedFamilies)for(const part of Object.values(family)){expect(part.tags).toContain('generated-source-sheet');expect(part.tags).toContain('vectorized-v2');expect(part.triangles.length).toBeGreaterThan(0);for(const triangle of part.triangles){expect(triangle.points).toHaveLength(3);expect(triangle.points.flat().every(Number.isFinite)).toBe(true);expect(Number.isFinite(triangle.shade??0)).toBe(true);}}
  });

  it('preserves the semantic layers that exist in the generated artwork',()=>{
