...
describe('sample-derived polygon geometry',()=>{
  ...
  it('restores source-sheet eye painter layers and measured aspect ratios',()=>{
    ...
    for(const [id,eye] of Object.entries(EYE_PARTS)){
      ...
      if(id!=='closed'){for(const layer of ['eye-white','iris','pupil'])expect(eye.triangles.some(t=>t.layer===layer)).toBe(true);}
    }
    expect(EYE_PARTS.bright.triangles.some(t=>t.layer==='eye-glint')).toBe(true);expect(EYE_PARTS.sparkle.triangles.some(t=>t.layer==='eye-glint')).toBe(true);
  });
