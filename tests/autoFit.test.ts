...
function combination(index:number){
  ...
  c.outfitStyle=outfits[rotate(outfits.length,index,5,0)];c.hoodStyle=hoods[rotate(hoods.length,index,5,1)];c.shirtStyle=shirts[rotate(shirts.length,index,1,2)];c.strapStyle=straps[rotate(straps.length,index,5,3)];c.accentStyle=accents[rotate(accents.length,index,3,1)];
  c.hairStyle=hair[rotate(hair.length,index,3,0)];c.faceShape=faces[rotate(faces.length,index,7,1)];c.eyeStyle=eyes[rotate(eyes.length,index,9,2)];c.browStyle=brows[rotate(brows.length,index,1,3)];c.noseStyle=noses[rotate(noses.length,index,3,4)];c.mouthStyle=mouths[rotate(mouths.length,index,7,5)];return c;
}
...
describe('92-part deterministic auto-fit',()=>{
  ...
  it('uses a deterministic semantic z-order instead of source-sheet insertion order',()=>{
    ...
    expect(z.shirt).toBe(LAYER_Z.shirt);expect(z.jacket).toBe(LAYER_Z.jacket);expect(z.hood).toBe(LAYER_Z.hood);expect(z.strap).toBe(LAYER_Z.strap);expect(z.accent).toBe(LAYER_Z.accent);expect(z.shirt).toBeLessThan(z.jacket);expect(z.jacket).toBeLessThan(z.hood);expect(z.hood).toBeLessThan(z.strap);expect(z.strap).toBeLessThan(z.accent);expect(z.face).toBeLessThan(z['eye-outline']);expect(z['eye-glint']).toBeLessThan(z.brows);expect(z.brows).toBeLessThan(z['hair-front']);for(let i=1;i<mesh.layers.length;i++)expect(mesh.layers[i].zIndex).toBeGreaterThanOrEqual(mesh.layers[i-1].zIndex);
  });
  ...
  it('keeps user Mii-style adjustments as offsets on top of automatic placement',()=>{
    ...
    expect(leftAfter.transform.x).toBeLessThan(leftBefore.transform.x);expect(leftAfter.transform.y).toBeGreaterThan(leftBefore.transform.y);expect(leftAfter.score).toBeGreaterThan(leftBefore.score);const mouthBefore=before.entries.find(e=>e.family==='mouth')!,mouthAfter=after.entries.find(e=>e.family==='mouth')!;expect(mouthAfter.transform.scaleX).toBeGreaterThan(mouthBefore.transform.scaleX);expect(mouthAfter.score).toBeGreaterThanOrEqual(mouthBefore.score);
  });
});
