...
function combination(index:number){
  ...
  c.outfitStyle=outfits[index%outfits.length];c.hoodStyle=hoods[index%hoods.length];c.shirtStyle=shirts[index%shirts.length];c.strapStyle=straps[index%straps.length];c.accentStyle=accents[index%accents.length];
  return c;
}
...
describe('92-part deterministic auto-fit',()=>{
  it('covers every generated selectable part in ten full-character audit combinations',()=>{
    ...
    for(let i=0;i<10;i++){
      ...
      for(const entry of report.entries){expect(finiteTransform(entry.transform)).toBe(true);expect(Number.isFinite(entry.score)).toBe(true);const id=entry.id.replace(/:(left|right)$/,'');seen.add(`${entry.family}:${id}`);}
      expect(report.entries.find(entry=>entry.family==='hair')!.score).toBeLessThan(4);
  ...
  it('uses a deterministic semantic z-order instead of source-sheet insertion order',()=>{
    ...
    expect(z.shirt).toBeLessThan(z.jacket);expect(z.jacket).toBeLessThan(z.hood);expect(z.hood).toBeLessThan(z.strap);expect(z.strap).toBeLessThan(z.accent);
    expect(z.face).toBeLessThan(z['eye-outline']);expect(z['eye-glint']).toBeLessThan(z.brows);expect(z.brows).toBeLessThan(z['hair-front']);
  ...
  it('keeps user Mii-style adjustments as offsets on top of automatic placement',()=>{
    ...
    expect(leftAfter.transform.x).toBeLessThan(leftBefore.transform.x);expect(leftAfter.transform.y).toBeGreaterThan(leftBefore.transform.y);
    const mouthBefore=before.entries.find(e=>e.family==='mouth')!,mouthAfter=after.entries.find(e=>e.family==='mouth')!;expect(mouthAfter.transform.scaleX).toBeGreaterThan(mouthBefore.transform.scaleX);
  });
});
