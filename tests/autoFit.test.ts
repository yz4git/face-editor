...
describe('92-part deterministic auto-fit',()=>{
  it('covers all 92 parts across twenty rotated full-character combinations',()=>{
    const seen=new Set<string>();for(let i=0;i<20;i++){
      const c=combination(i),report=getCharacterAutoFitReport(c);expect(report.version).toBe(2);expect(report.entries).toHaveLength(13);expect((report.issues??[]).filter(issue=>issue.severity==='error')).toEqual([]);expect(Number.isFinite(report.totalScore)).toBe(true);
