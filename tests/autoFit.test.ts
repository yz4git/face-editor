...
describe('92-part deterministic auto-fit',()=>{
  it('covers all 92 parts across twenty rotated full-character combinations',()=>{
    const seen=new Set<string>();for(let i=0;i<20;i++){
      const c=combination(i),report=getCharacterAutoFitReport(c);expect(report.version).toBe(2);expect(report.entries).toHaveLength(13);for(const entry of report.entries){expect(finiteTransform(entry.transform)).toBe(true);expect(Number.isFinite(entry.score)).toBe(true);expect(entry.family==='hair'?entry.score<4:entry.score<.8).toBe(true);seen.add(`${entry.family}:${entry.id.replace(/:(left|right)$/,'')}`);}
