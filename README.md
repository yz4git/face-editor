...
The audit then covers every cross-family value pair, rotates through real full-editor screenshots, and checks an iPhone-landscape viewport. Exhaustive geometry checking only runs in audit mode; normal editor sessions keep the same lightweight rendering path. See [`docs/auto-fit-audit.md`](docs/auto-fit-audit.md).
