...
export const GENERATED_VARIATION_SOURCE={kind:'generated-reference-sheet',hairCount:10,eyeCount:10,fitRevision:13,method:'source-sheet mask segmentation + feature-preserving hair Delaunay + face-relative calibration + source-contour eyes + source iris hulls + sampled default-eye silhouette + tuned sclera/pupil ratios'} as const;

export const HAIR_REFERENCE_BOUNDS:Record<HairStyleId,ReferenceBounds>={
  ponytail:{minX:-.82,maxX:1.35,minY:-.35,maxY:2.18},bob:{minX:-.80,maxX:.95,minY:-.10,maxY:1.95},'side-tail':{minX:-.82,maxX:1.45,minY:-.65,maxY:1.95},'twin-tail':{minX:-1.22,maxX:1.30,minY:-.45,maxY:2.05},braid:{minX:-.82,maxX:1.10,minY:-.85,maxY:2.05},long:{minX:-.78,maxX:1.05,minY:-.65,maxY:2.00},wavy:{minX:-.95,maxX:1.05,minY:-.45,maxY:2.00},'short-spike':{minX:-.95,maxX:1.05,minY:-.05,maxY:2.25},bun:{minX:-.85,maxX:1.05,minY:-.15,maxY:2.25},'half-up':{minX:-.90,maxX:1.15,minY:-.35,maxY:2.25},
};
export const HAIR_REFERENCE_FIT=HAIR_PACKED_FIT;

export const EYE_REFERENCE_BOUNDS:Record<EyeStyleId,ReferenceBounds>={
  bright:{minX:-.145,maxX:.145,minY:-.205,maxY:.205},determined:{minX:-.150,maxX:.150,minY:-.165,maxY:.165},sharp:{minX:-.160,maxX:.160,minY:-.165,maxY:.165},round:{minX:-.145,maxX:.145,minY:-.190,maxY:.190},soft:{minX:-.155,maxX:.155,minY:-.160,maxY:.160},sleepy:{minX:-.170,maxX:.170,minY:-.125,maxY:.125},sparkle:{minX:-.155,maxX:.155,minY:-.200,maxY:.200},closed:{minX:-.160,maxX:.160,minY:-.055,maxY:.055},narrow:{minX:-.175,maxX:.175,minY:-.115,maxY:.115},'side-glance':{minX:-.155,maxX:.155,minY:-.150,maxY:.150},
};
...
const unitPoint=(p:Vec2,to:ReferenceBounds):Vec2=>[to.minX+p[0]*(to.maxX-to.minX),to.minY+p[1]*(to.maxY-to.minY)];
...
function triangulatePolygon(points:readonly Vec2[]):readonly [Vec2,Vec2,Vec2][]{if(points.length<3)return[];if(points.length===3)return[[points[0],points[1],points[2]]];const indices=points.map((_,i)=>i),ccw=signedArea(points)>0,out:[Vec2,Vec2,Vec2][]=[];let guard=0;while(indices.length>3&&guard++<points.length*points.length){let clipped=false;for(let i=0;i<indices.length;i++){const ia=indices[(i-1+indices.length)%indices.length],ib=indices[i],ic=indices[(i+1)%indices.length],a=points[ia],b=points[ib],c=points[ic],turn=cross(a,b,c);if(ccw?turn<=1e-7:turn>=-1e-7)continue;let contains=false;for(const k of indices){if(k===ia||k===ib||k===ic)continue;if(inTriangle(points[k],a,b,c)){contains=true;break;}}if(contains)continue;out.push([a,b,c]);indices.splice(i,1);clipped=true;break;}if(!clipped)break;}if(indices.length===3)out.push([points[indices[0]],points[indices[1]],points[indices[2]]]);if(!out.length){const center:Vec2=[points.reduce((s,p)=>s+p[0],0)/points.length,points.reduce((s,p)=>s+p[1],0)/points.length];return points.map((p,i)=>[center,p,points[(i+1)%points.length]] as [Vec2,Vec2,Vec2]);}return out;}
...
const WHITE_INSET:Record<EyeStyleId,readonly [number,number,number]>={bright:[.94,.91,-.002],determined:[.93,.84,-.002],sharp:[.93,.82,-.002],round:[.94,.91,-.002],soft:[.94,.86,-.002],sleepy:[.95,.78,-.003],sparkle:[.94,.91,-.002],closed:[0,0,0],narrow:[.95,.76,-.002],'side-glance':[.96,.86,-.002]};
const PUPIL_SCALE:Record<EyeStyleId,readonly [number,number]>={bright:[.50,.62],determined:[.50,.64],sharp:[.50,.64],round:[.52,.64],soft:[.50,.62],sleepy:[.52,.60],sparkle:[.48,.60],closed:[0,0],narrow:[.52,.60],'side-glance':[.50,.62]};
function contourEye(id:EyeStyleId):GeneratedVariantTriangle<GeneratedEyeRole>[] {
  ...
  out.push(...polygonTriangles('outline',outer));if(id==='closed')return out;
  ...
  if(iris.length){const [px,py]=PUPIL_SCALE[id],targetCenter=unitPoint(irisData.pupilCenter,to),base=scalePolygon(iris,px,py),baseCenter=centroid(base),pupil=translatePolygon(base,targetCenter[0]-baseCenter[0],targetCenter[1]-baseCenter[1]);out.push(...polygonTriangles('pupil',pupil));}
  if(iris.length){const b=boundsOfPoints(iris),w=b.maxX-b.minX,h=b.maxY-b.minY,cx=b.minX+w*.31,cy=b.maxY-h*.20;if(id==='sparkle')out.push(...starTriangles(cx,cy,Math.min(w,h)*.23,.28,4),...starTriangles(b.minX+w*.70,b.minY+h*.31,Math.min(w,h)*.12,.28,4));else out.push(...ellipseTriangles(cx,cy,w*(id==='side-glance'?.10:.12),h*.14,10));}
  return out;
}
