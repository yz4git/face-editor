...
import { ACCENT_COLORS, EAR_ACCESSORY_OPTIONS, EYEWEAR_OPTIONS, FACE_DETAIL_OPTIONS, HAIR_BACK_OPTIONS, HAIR_EXTRA_OPTIONS, HEADWEAR_OPTIONS, SHIRT_COLORS, TRIM_COLORS, normalizeAccessories, normalizeClothingLayers, normalizeHairModular, setAccessories, setHairModular } from './characterExpansion';
...
const maybePick=<T extends string>(none:T,items:readonly T[],noneChance:number,rng:()=>number)=>rng()<noneChance?none:pick(items,rng);
...
const STYLE_HAIR:Record<FactoryStyleId,{back:readonly string[];extra:readonly string[]}>={
  soft:{back:['auto','medium','wavy','long'],extra:['none','none','side-tail','bun']},
...
export function applyGeneratedExpansion(definition:CharacterDefinition,seed:string,style:FactoryStyleId='cool',anchor?:CharacterDefinition,locks:readonly FactoryLock[]=[]){
  ...
  if(lockSet.has('hair')&&anchor)setHairModular(out,normalizeHairModular(anchor));
    ...
    const hair=STYLE_HAIR[style],backs=hair.back.filter(id=>HAIR_BACK_OPTIONS.some(item=>item.id===id)) as ReturnType<typeof normalizeHairModular>['back'][],extras=hair.extra.map(id=>id==='side-tail'?'ponytail':id).filter(id=>HAIR_EXTRA_OPTIONS.some(item=>item.id===id)) as ReturnType<typeof normalizeHairModular>['extra'][];
