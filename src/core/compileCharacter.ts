...
function emitAccent(d:Drafts,c:CharacterDefinition,id:AccentStyleId){const fit=ACCENT_PHASE2_AUTO_FIT[id];for(const item of ACCENT_PARTS[id].triangles){if(id==='triangle'&&Math.max(...item.points.map(point=>point[1]))>TRIANGLE_ACCENT_MAX_Y)continue;d.tri(item.layer,item.zIndex,item.points.map(point=>apply(point,fit)),roleColor(item.colorRole,c,item.shade??0));}}
function emitOutfit(d:Drafts,c:CharacterDefinition){
  const id=c.outfitStyle??'hooded',outfitFit=OUTFIT_PHASE2_AUTO_FIT[id];emitOutfitUnderlay(d,c,id);
  ...
  emitPart(d,c,SHIRT_PARTS[shirtId],SHIRT_PHASE2_AUTO_FIT[shirtId]);
  emitPart(d,c,HOOD_PARTS[hoodId],HOOD_PHASE2_AUTO_FIT[hoodId]);
  emitPart(d,c,STRAP_PARTS[strapId],STRAP_PHASE2_AUTO_FIT[strapId]);
...
export function compileCharacter(c:CharacterDefinition):CompiledPolygonCharacter{
  const d=new Drafts();emitSkinUnderlay(d,c);emitPart(d,c,BODY_PARTS[c.baseStyle??'female']);emitOutfit(d,c);emitPart(d,c,FACE_PARTS[c.faceShape],FACE_PHASE2_AUTO_FIT[c.faceShape]);emitHairUnderCap(d,c);const hairFit=composeAxisAlignedTransforms(HAIR_SOURCE_FIT[c.hairStyle],HAIR_PHASE2_AUTO_FIT[c.hairStyle]);emitPart(d,c,HAIR_PARTS[c.hairStyle],hairFit);
