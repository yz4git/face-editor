...
export const HAIR_BACK_OPTIONS:readonly {id:HairBackStyleId;label:string}[]=[{id:'auto',label:'AUTO'},{id:'short',label:'SHORT'},{id:'medium',label:'MEDIUM'},{id:'long',label:'LONG'},{id:'wavy',label:'WAVY'}];
export const HAIR_EXTRA_OPTIONS:readonly {id:HairExtraStyleId;label:string}[]=[{id:'none',label:'NONE'},{id:'ponytail',label:'PONYTAIL'},{id:'twin-tail',label:'TWIN TAIL'},{id:'bun',label:'BUN'},{id:'braid',label:'BRAID'}];

export const HEADWEAR_OPTIONS:readonly {id:HeadwearStyleId;label:string}[]=[{id:'none',label:'NONE'},{id:'cap',label:'CAP'},{id:'beanie',label:'BEANIE'},{id:'beret',label:'BERET'},{id:'headband',label:'HEADBAND'},{id:'headphones',label:'HEADPHONES'},{id:'goggles-up',label:'GOGGLES UP'},{id:'small-crown',label:'CROWN'},{id:'sci-fi-visor',label:'SCI-FI VISOR'}];
export const EYEWEAR_OPTIONS:readonly {id:EyewearStyleId;label:string}[]=[{id:'none',label:'NONE'},{id:'round-glasses',label:'ROUND'},{id:'square-glasses',label:'SQUARE'},{id:'thin-frame',label:'THIN FRAME'},{id:'sunglasses',label:'SUNGLASSES'},{id:'monocle',label:'MONOCLE'},{id:'sport-goggles',label:'SPORT GOGGLES'},{id:'cyber-visor',label:'CYBER VISOR'},{id:'eyepatch',label:'EYEPATCH'}];
export const FACE_DETAIL_OPTIONS:readonly {id:FaceDetailStyleId;label:string}[]=[{id:'none',label:'NONE'},{id:'mole',label:'MOLE'},{id:'freckles',label:'FRECKLES'},{id:'blush',label:'BLUSH'},{id:'scar',label:'SCAR'},{id:'bandage',label:'BANDAGE'},{id:'face-paint',label:'FACE PAINT'},{id:'cheek-mark',label:'CHEEK MARK'},{id:'under-eye-line',label:'UNDER-EYE'}];
export const EAR_ACCESSORY_OPTIONS:readonly {id:EarAccessoryStyleId;label:string}[]=[{id:'none',label:'NONE'},{id:'stud-earring',label:'STUD'},{id:'hoop-earring',label:'HOOP'},{id:'ear-cuff',label:'EAR CUFF'},{id:'double-earring',label:'DOUBLE'},{id:'chain-earring',label:'CHAIN'},{id:'comms-device',label:'COMMS'},{id:'cyber-earpiece',label:'CYBER EAR'},{id:'star-earring',label:'STAR'}];

export const DEFAULT_SHIRT_COLOR='#16212b';
export const DEFAULT_TRIM_COLOR='#f3eee4';
export const DEFAULT_SECONDARY_COLOR='#6b4529';
export const DEFAULT_HARDWARE_COLOR='#d0ccc4';
...
export const SECONDARY_COLORS=['#6b4529','#1f2933','#4f5d69','#72444f','#3f5a46','#283d55','#8a6b3b','#c8b7a3'];
export const HARDWARE_COLORS=['#d0ccc4','#8b939b','#454b52','#d9b65a','#c97963','#7296aa','#e6e8eb','#92734d'];
...
const TOP_EXTRA_CONFLICTS:Readonly<Record<string,HairExtraStyleId>>={ponytail:'ponytail','side-tail':'ponytail','twin-tail':'twin-tail',bun:'bun',braid:'braid'};
...
export function setHairModular(definition:CharacterDefinition,state:HairModularState){const normalized={...state};if(hairExtraConflictsWithTop(definition,normalized.extra))normalized.extra='none';(definition as ExpandedCharacterDefinition).hairModular=structuredClone(normalized);return definition;}
...
export function normalizeAccessories(definition:CharacterDefinition):AccessoryState{
  ...
  return{headwear:value&&HEADWEAR_OPTIONS.some(item=>item.id===value.headwear)?value.headwear:'none',eyewear:value&&EYEWEAR_OPTIONS.some(item=>item.id===value.eyewear)?value.eyewear:'none',faceDetail:value&&FACE_DETAIL_OPTIONS.some(item=>item.id===value.faceDetail)?value.faceDetail:'none',earAccessory:value&&EAR_ACCESSORY_OPTIONS.some(item=>item.id===value.earAccessory)?value.earAccessory:'none'};
}
...
export function secondaryColor(definition:CharacterDefinition){return definition.colors.secondary??DEFAULT_SECONDARY_COLOR;}
export function hardwareColor(definition:CharacterDefinition){return definition.colors.hardware??DEFAULT_HARDWARE_COLOR;}
